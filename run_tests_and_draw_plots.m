...
for comparisoncount=1:3                      
    ...
    % Let's check natilarities:

    [speakers,IA,IC] = unique(natspeakers, 'stable');

    mins=min([natnons{1},natsigs{1}]');
    maxs=max([natnons{1},natsigs{1}]');

    for n=2:length(natnons)
       if ~isempty(natnons{n}) &&  ~isempty(natsigs{n})
       mins=min([mins',natnons{n},natsigs{n}]');
       maxs=max([maxs',natnons{n},natsigs{n}]');
       end
    end
    ...
    handbookplottables = cell(1,size(natsigs{1},1));
    handbookcurves = cell(1,size(natsigs{1},1));
    for feat=1:size(natsigs{1},1);    
        ...
        if sum((featlist==feat))==1
            ...
            title(['natilarity with feature ',num2str(feat),': ',testlist{feat}, ' ',comparisonname],'FontSize',18);
...
%                 plot(logvals-binstepsize/2,100*accum,'ro-');
    ...
    for featc=1:length(featlist);
        ...
        title(['natilarity with feature ',num2str(feat),': ',testlist{feat}],'FontSize',18);
