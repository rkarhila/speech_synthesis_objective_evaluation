...
switch ds_type
    ...
    case 'log-mel'
        for i=1:nr_frames_test
            logmel_test=feas_test(i,:);
            for j=1:nr_frames_ref

                logmel_ref=feas_ref(j,:);
                distmap(i,j)=sqrt(2*sum(power(logmel_test(2:min(mel_dim,cep_dim+1))-logmel_ref(2:min(mel_dim,cep_dim+1)),2)));
...
if sum(isnan(distmap(:)))>0
    disp(['Trouble: Contains NaNs; Changing to zeros. Analysis: ',ds_type]);
    distmap(isnan(distmap))=0;            
end
if min(distmap(:))<0
    disp(['Trouble: Contains negative numbers; Adding minimum value to map. Analysis: ',ds_type]);
